...
if(((Get-ComputerInfo).OSName) -eq "Microsoft Windows 10 Enterprise"){
    Write-Host "Windows 10 Machine"
    $Win10Machine = $true
    $Win11Machine = $false
}elseif(((Get-ComputerInfo).OSName) -eq "Microsoft Windows 11 Enterprise"){
    Write-Host "Windows 11 Machine"
    $Win10Machine = $false
    $Win11Machine = $true
}
...
$GoodApps =	"calculator|sticky|camera|microphone|photos|screensketch|soundrecorder|mspaint|store|search|VCLibs|Native"
  ...
  <DefaultLayoutOverride>
    <StartLayoutCollection>
      <defaultlayout:StartLayout GroupCellWidth="6">
        ...
        <start:Group Name="ACG">
          <start:DesktopApplicationTile Size="2x2" Column="0" Row="0" DesktopApplicationLinkPath="%ALLUSERSPROFILE%\Microsoft\Windows\Start Menu\Programs\Microsoft Endpoint Manager\Configuration Manager\Software Center.lnk" />
        </start:Group>
...
Function DisableServices {
    If ($LeaveServices) {
		...
		Get-Service Diagtrack,WMPNetworkSvc,irmon,SSDPSRV,upnphost,icssvc,XboxGipSvc,XblGameSave,XblAuthManager,XboxNetApiSvc -erroraction silentlycontinue | stop-service -passthru | set-service -startuptype disabled
...
#Custom start layout XML near the top of the script.
        if($Win10Machine -eq $true){
            Write-Host "*** Windows 10 Start Menu Layout Customization ***"
            add-content $Env:TEMP"\startlayout.xml" $StartLayoutStr
            import-startlayout -layoutpath $Env:TEMP"\startlayout.xml" -mountpath $Env:SYSTEMDRIVE\
            remove-item $Env:TEMP"\startlayout.xml"
        }elseif($Win11Machine -eq $true){
            Write-Host "*** Windows 11 Start Menu Layout Customization ***"
            add-content $Env:TEMP"\LayoutModification.json" $StartLayoutStr
            Copy-Item -Path $Env:TEMP"\LayoutModification.json" -Destination $env:SystemDrive"\Users\Default\Appdata\Local\Microsoft\Windows\Shell"
            Remove-Item -Path $Env:TEMP"\LayoutModification.json"
        }
    }
}
